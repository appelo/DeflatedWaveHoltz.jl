...
function run_case(fname,omega,nev)
    ...
    return history, length(res_udcg), length(log1.data[:resnorm]), length(log_2.data[:resnorm]), length(res_whi),mm
end


function loop_over_omega()

    results = zeros(2,8)
    idx = 1
    for om = 5:6
        omega = 1.0*om
        nev = Int(floor(1.5*omega))
        fname = @sprintf("om_%03d_nev%03d.jld2", om,nev)
        h,i1,i2,i3,i4,ACR = run_case(fname,omega,nev)
        results[idx,1] = h.mvproducts
        results[idx,2] = i1
        results[idx,3] = i2
        results[idx,4] = i3
        results[idx,5] = i4
    ...
    jldsave("results.jld2";results = results)
    return results
end
