function run_example_from_file(omega, xmap::Function, ymap::Function,qmin,qmax,rmin,rmax,order,ep_tol,
    ...
    # Symmetric operator for M*(1-S) CG
    S_whi_im!(y,x) = S_WHI_sym_operator_homi!(y,x,DP)
    ...
    force = zeros(DP.N)
    force .= DP.force
    QTQ = transpose(Q)*DP.Mass*Q
    coeff = QTQ \ (transpose(Q)*DP.Mass*force)
    ...
    for iter = 1:length(log1.data[:resnorm])
        ...
        # coe = QTQ \ (transpose(Q)*DP.Mass*uwhi)
        # uwhi .= uwhi .- Q*coe
...
function find_deflate(omega, xmap::Function, ymap::Function,qmin,qmax,rmin,rmax,order,ep_tol,
    ...
    ALM = LinearMap(A_whi_im!,DP.N,DP.N,issymmetric = false, ismutating=true, isposdef=false)
...
function process_deflation_from_file(fname,svd_tol=1e-12)
